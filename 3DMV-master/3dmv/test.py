...
# data paths
parser.add_argument(
    "--scene_list", required=True, help="path to file list of scenes to test"
)
...
parser.add_argument(
    "--has_gt", type=int, default=0, help="test scenes have gt to evaluate against"
)
...
model2d_fixed, model2d_trainable, model2d_classifier = create_enet_for_3d(
    ENET_TYPES[opt.model2d_type], opt.model2d_orig_path, num_classes
)
...
def evaluate_prediction(scene_occ, scene_label, output):
    ...
    num_wrong = np.count_nonzero(
        scene_label.astype(np.int32)[mask] - output.astype(np.int32)[mask]
    )
    ...
    for c in range(num_classes):
        ...
        class_num_union[c] = np.count_nonzero(
            np.logical_or(
                mask,
                np.logical_and(
                    np.not_equal(scene_label, num_classes - 1), np.equal(output, c)
                ),
            )
        )
    ...
    return {
        "instance_num_correct": inst_num_correct,
        "instance_num_total": inst_num_occ,
        "class_num_correct": class_num_correct,
        "class_num_total": class_num_occ,
...
def test(scene_name, eval_file):
    ...
    (
        ...
        frame_ids,
        world_to_grids,
    ) = data_util.load_scene_image_info_multi(
        scene_image_file,
        scene_name,
        opt.data_path_2d,
        proj_image_dims,
    ...
    depth_image = torch.cuda.FloatTensor(
        num_images, proj_image_dims[1], proj_image_dims[0]
    )
    color_image = torch.cuda.FloatTensor(
        num_images, 3, input_image_dims[1], input_image_dims[0]
    )
    ...
    for y in range(grid_padY, scene_occ_sz[1] - grid_padY):
        for x in range(grid_padX, scene_occ_sz[2] - grid_padX):
            ...
            if (
                len(cur_frame_ids) < num_images
                or torch.sum(input_occ[0, 0, :, grid_padY, grid_padX]) == 0
            ):
                continue
            for k in range(num_images):
            ...
            out = model(
                torch.autograd.Variable(input_occ),
                imageft,
    ...
    util.write_array_to_file(
        pred_label.astype(np.uint8), os.path.join(opt.output_path, scene_name + ".bin")
    )
    eval_scene = None
    if opt.has_gt:
