...
ENET_TYPES = {
    "scannet": (41, [0.496342, 0.466664, 0.440796], [0.277856, 0.28623, 0.291129])
...
# data paths
parser.add_argument(
    "--train_data_list", required=True, help="path to file list of h5 train data"
)
parser.add_argument(
    "--val_data_list", default="", help="path to file list of h5 val data"
)
parser.add_argument(
    "--output", default="./logs", help="folder to output model checkpoints"
)
parser.add_argument("--data_path_2d", required=True, help="path to 2d train data")
parser.add_argument(
    "--class_weight_file", default="", help="path to histogram over classes"
)
# train params
...
parser.add_argument(
    "--max_epoch", type=int, default=20, help="number of epochs to train for"
)
parser.add_argument(
...
parser.add_argument(
    "--weight_decay", type=float, default=0.0005, help="weight decay, default=0.0005"
)
...
# 2d/3d
parser.add_argument(
    "--voxel_size", type=float, default=0.05, help="voxel size (in meters)"
)
...
parser.add_argument(
    "--depth_min", type=float, default=0.4, help="min depth (in meters)"
)
parser.add_argument(
    "--depth_max", type=float, default=4.0, help="max depth (in meters)"
)
# scannet intrinsic params
parser.add_argument(
    "--intrinsic_image_width", type=int, default=640, help="2d image width"
)
parser.add_argument(
    "--intrinsic_image_height", type=int, default=480, help="2d image height"
)
...
intrinsic = util.adjust_intrinsic(
    intrinsic, [opt.intrinsic_image_width, opt.intrinsic_image_height], proj_image_dims
)
...
projection = ProjectionHelper(
    intrinsic, opt.depth_min, opt.depth_max, proj_image_dims, grid_dims, opt.voxel_size
)
# create loss
criterion_weights = torch.ones(num_classes)
if opt.class_weight_file:
    criterion_weights = util.read_class_weights_from_file(
        opt.class_weight_file, num_classes, True
    )
...
optimizer2d = torch.optim.SGD(
    model2d_trainable.parameters(),
    lr=opt.lr,
    momentum=opt.momentum,
    weight_decay=opt.weight_decay,
)
if opt.use_proxy_loss:
    optimizer2dc = torch.optim.SGD(
        model2d_classifier.parameters(),
        lr=opt.lr,
        momentum=opt.momentum,
        weight_decay=opt.weight_decay,
    )
...
val_files = (
    [] if not opt.val_data_list else util.read_lines_from_file(opt.val_data_list)
)
...
def train(epoch, iter, log_file, train_file, log_file_2d):
    ...
    depth_images = torch.cuda.FloatTensor(
        batch_size * num_images, proj_image_dims[1], proj_image_dims[0]
    )
    color_images = torch.cuda.FloatTensor(
        batch_size * num_images, 3, input_image_dims[1], input_image_dims[0]
    )
    camera_poses = torch.cuda.FloatTensor(batch_size * num_images, 4, 4)
    label_images = torch.cuda.LongTensor(
        batch_size * num_images, proj_image_dims[1], proj_image_dims[0]
    )
    ...
    for t, v in enumerate(indices):
        ...
        transforms = (
            transforms.expand(batch_size, num_images, 4, 4)
            .contiguous()
            .view(-1, 4, 4)
            .cuda()
        )
        data_util.load_frames_multi(
            opt.data_path_2d,
            frames[v],
            depth_images,
            color_images,
        ...
        if opt.use_proxy_loss:
            data_util.load_label_frames(
                opt.data_path_2d, frames[v], label_images, num_classes
            )
        ...
        output = model(
            ...
            torch.autograd.Variable(proj_ind_2d),
            grid_dims,
        )
        ...
        confusion.add(
            torch.index_select(predictions, 0, maskindices),
            torch.index_select(k, 0, maskindices),
        )
        log_file.write(
            _SPLITTER.join([str(f) for f in [epoch, iter, loss.item()]]) + "\n"
        )
        iter += 1
        if iter % 10000 == 0:
            torch.save(
                model.state_dict(),
                os.path.join(opt.output, "model-iter%s-epoch%s.pth" % (iter, epoch)),
            )
            torch.save(
            ...
            if opt.use_proxy_loss:
                torch.save(
                    ...
                    os.path.join(
                        opt.output, "model2dc-iter%s-epoch%s.pth" % (iter, epoch)
                    ),
                )
        if iter == 1:
            torch.save(
                model2d_fixed.state_dict(), os.path.join(opt.output, "model2dfixed.pth")
            )

        if iter % 100 == 0:
            evaluate_confusion(
                confusion, train_loss, epoch, iter, -1, "Train", log_file
            )
            if opt.use_proxy_loss:
                evaluate_confusion(
                    confusion2d, train_loss_2d, epoch, iter, -1, "Train2d", log_file_2d
                )
    ...
    if opt.use_proxy_loss:
        evaluate_confusion(
            confusion2d, train_loss_2d, epoch, iter, took, "Train2d", log_file_2d
        )
...
def test(epoch, iter, log_file, val_file, log_file_2d):
    ...
    if opt.use_proxy_loss:
        evaluate_confusion(
            confusion2d_val, test_loss_2d, epoch, iter, took, "Test2d", log_file_2d
        )
...
def evaluate_confusion(confusion_matrix, loss, epoch, iter, time, which, log_file):
    ...
    avg_acc = (
        -1
        if np.all(np.equal(valids, -1))
        else np.mean(valids[np.not_equal(valids, -1)])
    )
    log_file.write(
        _SPLITTER.join(
            [
    ...
    print(
        "{} Epoch: {}\tIter: {}\tLoss: {:.6f}\tAcc(inst): {:.6f}\tAcc(avg): {:.6f}\tTook: {:.2f}".format(
            which,
            epoch,
            iter,
            torch.mean(torch.Tensor(loss)),
...
def main():
    ...
    log_file.write(
        _SPLITTER.join(["epoch", "iter", "loss", "avg acc", "instance acc", "time"])
        + "\n"
    )
    log_file.flush()
    log_file_2d = None
    ...
    if has_val:
        ...
        if opt.use_proxy_loss:
            ...
            log_file_2d_val.write(
                _SPLITTER.join(
                    ["epoch", "iter", "loss", "avg acc", "instance acc", "time"]
                )
                + "\n"
            )
    ...
    for epoch in range(opt.max_epoch):
        ...
        for k in range(len(train_file_indices)):
            print(
                "Epoch: {}\tFile: {}/{}\t{}".format(
                    epoch, k, len(train_files), train_files[train_file_indices[k]]
                )
            )
            loss, iter, loss2d = train(
                epoch, iter, log_file, train_files[train_file_indices[k]], log_file_2d
            )
            ...
            if has_val and k % num_files_per_val == 0:
                ...
                loss, loss2d = test(
                    epoch, iter, log_file_val, val_files[val_index], log_file_2d_val
                )
        ...
        if opt.use_proxy_loss:
            evaluate_confusion(
                confusion2d, train2d_loss, epoch, iter, -1, "Train2d", log_file_2d
            )
        if has_val:
            evaluate_confusion(
                confusion_val, val_loss, epoch, iter, -1, "Test", log_file_val
            )
            if opt.use_proxy_loss:
                evaluate_confusion(
                    confusion2d_val,
        ...
        torch.save(
            model.state_dict(), os.path.join(opt.output, "model-epoch-%s.pth" % epoch)
        )
        torch.save(
            model2d_trainable.state_dict(),
            os.path.join(opt.output, "model2d-epoch-%s.pth" % epoch),
        )
        if opt.use_proxy_loss:
            torch.save(
                model2d_classifier.state_dict(),
                os.path.join(opt.output, "model2dc-epoch-%s.pth" % epoch),
            )
        confusion.reset()
        confusion2d.reset()
        confusion_val.reset()
