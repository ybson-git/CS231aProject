...
def load_depth_label_pose(
    depth_file, color_file, pose_file, depth_image_dims, color_image_dims, normalize
):
...
def load_scene(filename, num_classes, load_gt):
    ...
    occ = np.ndarray(
        (2, depth, height, width), np.dtype("B")
    )  # occupancy grid for occupied/empty space, known/unknown space
    occ[0] = np.less_equal(np.abs(sdfs), 1)
    occ[1] = np.greater_equal(sdfs, -1)
    if load_gt:
        # ensure occupied space has non-zero labels
        labels[np.logical_and(np.equal(occ[0], 1), np.equal(labels, 0))] = (
            num_classes - 1
        )
...
def load_label_frames(data_path, frame_indices, label_images, num_classes):
    ...
    scan_names = [
        "scene" + str(scene_id).zfill(4) + "_" + str(scan_id).zfill(2)
        for scene_id, scan_id in frame_indices[:, :2].numpy()
    ]
    scan_names = np.repeat(scan_names, num_images)
    frame_ids = frame_indices[:, 2:].contiguous().view(-1).numpy()
    label_files = [
        os.path.join(data_path, scan_name, "label", str(frame_id) + ".png")
        for scan_name, frame_id in zip(scan_names, frame_ids)
    ]
...
def load_frames_multi(
    data_path, frame_indices, depth_images, color_images, poses, color_mean, color_std
):
    # construct files
    num_images = frame_indices.shape[1] - 2
    scan_names = [
    ...
    frame_ids = frame_indices[:, 2:].contiguous().view(-1).numpy()
    depth_files = [
        os.path.join(data_path, scan_name, "depth", str(frame_id) + ".png")
        for scan_name, frame_id in zip(scan_names, frame_ids)
    ]
    color_files = [
        os.path.join(data_path, scan_name, "color", str(frame_id) + ".jpg")
        for scan_name, frame_id in zip(scan_names, frame_ids)
    ]
    pose_files = [
        os.path.join(data_path, scan_name, "pose", str(frame_id) + ".txt")
        for scan_name, frame_id in zip(scan_names, frame_ids)
    ]
    ...
    # load data
    for k in range(batch_size):
        depth_image, color_image, pose = load_depth_label_pose(
            depth_files[k],
            color_files[k],
            pose_files[k],
            depth_image_dims,
...
def load_scene_image_info_multi(
    filename,
    scene_name,
    image_path,
    depth_image_dims,
    ...
    world_to_grids = struct.unpack(
        "f" * numElems, fin.read(numElems * 4)
    )  # grid2<mat4f>
    fin.close()
    frame_ids = np.asarray(frame_ids, dtype=np.int32).reshape(
        [max_num_images, height, width]
    )
    world_to_grids = np.asarray(world_to_grids, dtype=np.float32).reshape(
        [height, width, 4, 4]
    )
    # load data
