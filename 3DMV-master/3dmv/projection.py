...
class ProjectionHelper:
    def __init__(
        self, intrinsic, depth_min, depth_max, image_dims, volume_dims, voxel_size
    ):
    ...
    def compute_projection(self, depth, camera_to_world, world_to_grid):
        ...
        tmp = (
            lin_ind_volume
            - (coords[2] * self.volume_dims[0] * self.volume_dims[1]).long()
        )
        coords[1] = tmp / self.volume_dims[0]
        coords[0] = torch.remainder(tmp, self.volume_dims[0])
        coords[3].fill_(1)
        mask_frustum_bounds = (
            torch.ge(coords[0], voxel_bounds_min[0])
        ...
        lin_indices_2d[1 : 1 + lin_indices_2d[0]] = torch.index_select(
            valid_image_ind_lin, 0, torch.nonzero(depth_mask)[:, 0]
        )
...
class Projection(Function):
    ...
    def forward(ctx, label, lin_indices_3d, lin_indices_2d, volume_dims):
        ...
        output = label.new(
            num_label_ft, volume_dims[2], volume_dims[1], volume_dims[0]
        ).fill_(0)
        num_ind = lin_indices_3d[0]
        if num_ind > 0:
            vals = torch.index_select(
                label.view(num_label_ft, -1), 1, lin_indices_2d[1 : 1 + num_ind]
            )
    ...
    def backward(ctx, grad_output):
        ...
        vals = torch.index_select(
            grad_output.data.contiguous().view(num_ft, -1),
            1,
